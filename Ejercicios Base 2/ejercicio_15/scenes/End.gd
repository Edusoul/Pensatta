...
func _ready():
	...
	if global.timeout == false: #Generally timeout is false when there is no timer
		if global.correct_answers_no.size() == 7: #Change accordingly
			result_title.text = "¡Felicitaciones!"
			result_summary.text = "Respondiste todo bien."
		if global.correct_answers_no.size() == 6:
			result_title.text = "¡Muy Bien!"
			result_summary.text = "Respondiste 6 acertadamente."			
		if global.correct_answers_no.size() == 5:
			result_title.text = "¡Bien!"			
			result_summary.text = "Respondiste 5 acertadamente."
		if global.correct_answers_no.size() == 4:
			result_title.text = "¡Bien!"			
			result_summary.text = "Respondiste 4 acertadamente."
		if global.correct_answers_no.size() == 3:
			result_title.text = "¡Puedes mejorar!"
			result_summary.text = "Respondiste 3 correctas."
		if global.correct_answers_no.size() == 2:
			result_title.text = "¡Puedes mejorar!"
			result_summary.text = "Respondiste 2 correctas."						
		if global.correct_answers_no.size() == 1:
			result_title.text = "¡Continúa intentando!"
			result_summary.text = "Acertaste solo una."			
	else:
		result_title.text = "¡Continúa intentando!"
